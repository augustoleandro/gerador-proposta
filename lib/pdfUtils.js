...
Handlebars.registerHelper(
  "chunkWithLast",
  function (array, normalSize, lastSize) {
    const chunks = [];
    for (let i = 0; i < array.length; i += normalSize) {
      const remainingItems = array.length - i;
      if (remainingItems <= lastSize) {
        chunks.push(array.slice(i));
        break;
      } else {
        chunks.push(array.slice(i, i + normalSize));
      }
    }
    return chunks;
  }
...
Handlebars.registerHelper(
  "calculateItemNumber",
  function (chunkIndex, itemIndex, isLastChunk) {
    const chunkSize = isLastChunk ? 16 : 20;
    return chunkIndex * chunkSize + itemIndex + 1;
  }
);
